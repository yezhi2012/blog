...
function InitLoad() {
    setTimeout("LoadExportApiWordBtn()", 300);//加载导出按钮
    setTimeout("AddTokenEvent()", 300);//绑定事件增加Bearer 标识
}
...
function AddTokenEvent() {
    $(document).on("click",
        ...
        function () {
            console.log($(".auth-container").find("section").find("input"))
            $(".auth-container").find("section").find("input").val('Bearer ');// 加token标识
            $(".auth-container").find("section").find("input").focus();//获取焦点
        });
    $(document).on("click",
